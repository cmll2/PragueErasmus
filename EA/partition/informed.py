...
def informed_mutate_1(p, switch_bin_prob,weights):
    for i in range(len(p)):
        bw = bin_weights(weights, p)
        max_bin = np.argmax(bw)
        min_bin = np.argmin(bw)
        if p[i] == max_bin:
            if random.random() < switch_bin_prob:
                p[i] = min_bin
        elif p[i] == min_bin:
            if random.random() < switch_bin_prob:
                p[i] = max_bin
    return p
def informed_mutate(p, switch_bin_prob,weights):
    bw = bin_weights(weights, p)
    max_bin = np.argmax(bw)
    min_bin = np.argmin(bw)
    for i in range(len(p)):
...
def informed_mutate_2(p, switch_bin_prob,weights):
    bw = bin_weights(weights, p)
    sorted_indices = np.argsort(bw)
    length = len(sorted_indices)
    for i in range(len(p)):
        where = np.where(sorted_indices == p[i])[0][0]
        multiplicator = np.abs(where-length/2)*2
        if random.random() < multiplicator * switch_bin_prob:
            #find index of p[i] in sorted_indices
            index = where
            p[i] = sorted_indices[-index-1]
    return p

def informed_mutate_3(p, switch_bin_prob,weights):
    bw = bin_weights(weights, p)
    max_bin = np.argmax(bw)
    min_bin = np.argmin(bw)
    point1 = random.randrange(0, len(p))
...
def whole_mutation(p, flip_prob, switch_bin_prob,upper,weights):
    obj_before = fitness(p, weights).objective
    first_mutation = flip_mutate(p, flip_prob, upper)
    obj_first = fitness(first_mutation, weights).objective
    if obj_first > obj_before : pop = p
    else : 
        pop = first_mutation
        obj_before = obj_first
    second_mutation = informed_mutate(pop, switch_bin_prob, weights)
    obj_second = fitness(second_mutation, weights).objective
    if obj_second > obj_before:
        return pop
    else:
...
if __name__ == '__main__':
    ...
    mut = functools.partial(mutation, mut_prob=MUT_PROB, 
                            mutate=functools.partial(whole_mutation, flip_prob=MUT_FLIP_PROB, switch_bin_prob = SWITCH_BIN_PROB,upper=K, weights = weights), fitness=fit)
    ...
    utils.plot_experiment(evals, lower, mean, upper, legend_name = 'Mean objective value')
    plt.title('Convergence : Informed, 3pt-cross, 200 pop-size, 1000 gen')
    plt.yscale('log')
    plt.legend()
    plt.show()
