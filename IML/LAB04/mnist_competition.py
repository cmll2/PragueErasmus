...
def main(args: argparse.Namespace) -> Optional[npt.ArrayLike]:
    if args.predict is None:
        ...
        model = sklearn.neural_network.MLPClassifier(hidden_layer_sizes=(100,100), max_iter=100, verbose=True)
        model.fit(train.data, train.target)
        predictions = model.predict(train.data)
        print(sklearn.metrics.accuracy_score(train.target, predictions))
