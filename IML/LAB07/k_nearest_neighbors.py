...
def main(args: argparse.Namespace) -> float:
    ...
    for test in test_data:
        distances = np.linalg.norm(train_data - test, ord=args.p, axis=1)
        nearest = np.argsort(distances)[:args.k]
        nearest_targets = train_target[nearest]
        if args.weights == "uniform":
            ...
            raise ValueError("Unknown weighting scheme '{}'".format(args.weights))
        test_predictions.append(np.argmax(np.bincount(nearest_targets, weights=weights)))
