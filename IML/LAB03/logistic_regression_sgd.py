...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, train_data.shape[0], args.batch_size):
            ...
            weights -= args.learning_rate * (batch.T @ (batch_predict - batch_target) / batch.shape[0])
        ...
        train_predict = 1 / (1 + np.exp(-train_data @ weights))
        test_predict = 1 / (1 + np.exp(-test_data @ weights))
        train_loss = -np.mean(train_target * np.log(train_predict) + (1 - train_target) * np.log(1 - train_predict))
        test_loss = -np.mean(test_target * np.log(test_predict) + (1 - test_target) * np.log(1 - test_predict))
        train_accuracy = np.mean(train_target == (train_predict > 0.5))
        test_accuracy = np.mean(test_target == (test_predict > 0.5))
