...
def verify_with_dict(predicted, train, dict):
    predicted = predicted.split()
    train = train.split()
    for i in range(len(predicted)):
        if dict.get(train[i]) is not None:
            if predicted[i] in dict[train[i]]:
                continue
            else:
                predicted[i] = dict[train[i]][0]
        else:
            continue
    return ' '.join(predicted)
...
def main(args: argparse.Namespace) -> Optional[str]:
    if args.predict is None:
        ...
        predicted = verify_with_dict(predicted, test.data, dictionary.variants)
    return predicted
